...
export const DocumentPlateau = (props: IDocumentPlateauProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { documentPlateauList, match, loading } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {documentPlateauList && documentPlateauList.length > 0 ? (
          ...
          !loading && <div className="alert alert-warning">No Document Plateaus found</div>
        )}
      </div>
    </div>
  );
...
const mapStateToProps = ({ documentPlateau }: IRootState) => ({
  ...
  loading: documentPlateau.loading
});
...
export default connect(mapStateToProps, mapDispatchToProps)(DocumentPlateau);
